    ...
    const getRestaurantList = async () => {
        try {
            ...
            if (responseData.status === true) {
                const restaurantOnly = responseData.data.filter(item => item.accomodation_type === 'restaurant');
                setRestaurantList(restaurantOnly);

                const initialImageSelection = {};
                restaurantOnly.forEach(item => {
            ...
            >
                ...
                <View style={styles.headerContainer}>
                    <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 40, paddingHorizontal: 15 }}>
                        <View style={{ width: '75%' }}>
                            <Text style={{ color: '#fff', fontSize: 18, fontFamily: 'FiraSans-Regular' }}>Famous Restaurant In The City</Text>
                            <Text style={{ color: '#ddd', fontSize: 12, marginTop: 5, fontFamily: 'FiraSans-Regular' }}>Some Of The Luxurious & Good Restaurant In The City Of Puri</Text>
                        ...
                        renderItem={({ item, index }) => {
                            return (
                                <View>
                                    ...
                                    {index !== restaurantList.length - 1 && <View style={{ borderBottomWidth: 1, borderBottomColor: '#ddd', marginVertical: 20 }} />}
                                </View>
                            )
                        }}
                    />
